...
void AFMotorController::enable(void) {
  ...
  SER_DDR |= _BV(SER);
  */
  pinMode(MOTORLATCH, GPIO_DIR_MODE_OUT);
  pinMode(MOTORENABLE, GPIO_DIR_MODE_OUT);
  pinMode(MOTORDATA, GPIO_DIR_MODE_OUT);
  pinMode(MOTORCLK, GPIO_DIR_MODE_OUT);
...
inline void initPWM1(uint8_t freq) {
    // We are not using PWM for pin 11, then just do digital
    digitalWrite(sD11, LOW);
    pinMode(sD11, GPIO_DIR_MODE_OUT);
}
