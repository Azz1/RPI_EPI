...
extern "C" {
	...
	void Adafruit_MotorShield_Init() {
	...
	void Adafruit_MotorShield_SetSpeed(uint8_t motor, unsigned long speed) {

		Adafruit_StepperMotor *StepMotor = AFMS.getStepper(200, motor);
    	StepMotor->setSpeed(speed);
	}

	void Adafruit_MotorShield_Step(uint8_t motor, unsigned long totalsteps, unsigned char dir, unsigned char style) {

		Adafruit_StepperMotor *StepMotor = AFMS.getStepper(200, motor);
    	StepMotor->step(totalsteps, dir, style);
	}

	void Adafruit_MotorShield_Release(uint8_t motor) {
